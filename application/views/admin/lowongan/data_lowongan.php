						...
						foreach ($data as $x) { ?>
							<?php if ($x->batas_tanggal < date('Y-m-d')) {
                              ...
                              alt=" Sarana" class="card-img-top" data-holder-rendered="true" style="height: 275px; width: 225px; display: block;"></td>
									<td align="center">
										<a href="<?= base_url('mitra/edit_lowongan/' . $x->id_lowongan) ?>" class="btn btn-success">Ubah</a>

										<a href="<?= base_url('mitra/semua_pelamar_p/' . $x->id_lowongan) ?>" class="btn btn-primary">Lihat Pelamar</a>

										<a href="<?= base_url('mitra/hapus_lowongan/' . $x->id_lowongan) ?>" onclick="return confirm('Yakin Hapus?')" class="btn btn-danger">Hapus Lowongan</a>
